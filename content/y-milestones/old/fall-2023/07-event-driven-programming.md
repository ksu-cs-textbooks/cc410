...
* Each entree panel in `sfsubs.gui.entrees`
* Each drink panel in `sfsubs.gui.drinks`
* The side panel in `sfsubs.gui.sides`
...
* Classes and unit tests in the `sfsubs.gui` package and sub-packages **do require** all appropriate documentation comments, and must be free of style errors. **Every method must include full documentation comments.**
* Classes in the `sfsubs.gui` base package **do not require** unit tests, but all entree, drink, and side panels require unit tests as outlined above.
* Classes and unit tests in the `sfsubs.gui` package and sub-packages **do not require** type hints in Python, though you may continue to use them if they are helpful. Any errors from Mypy originating in these classes will be ignored.
...
<div>
{{% badge style="orange" %}}YELLOW: Limited GenAI Usage Allowed {{% /badge %}} For this project, you may make limited use of GenAI to answer questions and receive suggestions about how to add the requested functionality to your existing GUI. You <b>may not</b> use the results of GenAI directly in your code or documentation comments.
</div>
</div>
